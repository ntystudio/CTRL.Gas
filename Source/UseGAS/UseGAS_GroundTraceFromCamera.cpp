...
#include "UseGAS_GroundTraceFromCamera.h"
...
FHitResult AUseGAS_GroundTraceFromCamera::PerformTrace(AActor* InSourceActor)
{
	UE_LOG(LogTemp, Warning, TEXT("AUseGAS_GroundTraceFromCamera::PerformTrace %s"), *LexToString(bLastTraceWasGood));
	constexpr bool bTraceComplex = false;

	FCollisionQueryParams Params(SCENE_QUERY_STAT(AUseGAS_GroundTraceFromCamera), bTraceComplex);
	...
	if (CollisionShape.ShapeType != ECollisionShape::Line)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("AUseGAS_GroundTraceFromCamera::AdjustCollisionResultForShape %s"), *LexToString(bLastTraceWasGood));
...
void AUseGAS_GroundTraceFromCamera::Tick(float DeltaSeconds)
{
...
bool AUseGAS_GroundTraceFromCamera::ShouldProduceTargetData() const
{
	return Super::ShouldProduceTargetData();
}

bool AUseGAS_GroundTraceFromCamera::IsConfirmTargetingAllowed()
{
	if (!bHasTicked)
	{
		return true;
